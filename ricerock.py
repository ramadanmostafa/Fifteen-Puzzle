"""
ricerocks game developed by my during take a coursera course in rice university using python programming language
...
class Sprite:
    ...
    def draw(self, canvas):
        tmp = self.image_center[0]
        if self.animated:
            tmp += self.age * self.image_size[0]
        canvas.draw_image(self.image, [tmp,self.image_center[1]], self.image_size,
                          self.pos, self.image_size, self.angle)
...
# return the number of elements in the first group that collide with the 
#second group as well as delete these elements in the first group.
...
def rock_spawner():
    if len(rock_group) < 13:
        ...
        rock_vel = [random.random() * .6 - .3 + score/100, random.random() * .6 - .3 + score/100]
        rock_avel = random.random() * .2 - .1    
        if dist(rock_pos,my_ship.get_position()) > my_ship.get_radius() + asteroid_info.get_radius() + 30:
            a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
            rock_group.add(a_rock)
# take a set and a canvas and call the update and draw methods
